...
function cmplz_tc_editor_assets() { // phpcs:ignore
    ...
    wp_set_script_translations( 'cmplz-block', 'complianz-terms-conditions' , cmplz_tc_path . '/languages');

	// Styles.
	$load_css = cmplz_tc_get_value('use_document_css');
	if ($load_css) {
		wp_enqueue_style(
			'cmplz-block', // Handle.
			cmplz_tc_url . "assets/css/document.min.css",
			array( 'wp-edit-blocks' ), cmplz_tc_version
		);
	}
}
...
function cmplz_tc_render_document_block($attributes, $content)
{
    ...
    if (isset($attributes['selectedDocument'])) {
        if (isset($attributes['documentSyncStatus']) && $attributes['documentSyncStatus']==='unlink' && isset($attributes['customDocument'])){
	        ...
	        $region = cmplz_tc_get_region_from_legacy_type($type);
	        if ($region){
	        	$type = str_replace('-'.$region, '', $type);
	        }
            $html = COMPLIANZ_TC::$document->get_document_html($type, $region);
        }
    }
