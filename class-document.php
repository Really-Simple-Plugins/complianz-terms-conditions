...
if ( ! class_exists( "cmplz_tc_document" ) ) {
	class cmplz_tc_document {
		...
		public function condition_applies( $element, $post_id ) {
			if ( isset( $element['condition'] ) ) {
				...
				foreach (
					...
					$value = cmplz_tc_get_value( $question, $post_id );
		...
		private function get_plain_text_value( $fieldname, $post_id, $list_style, $type ) {
			...
			$front_end_label
				= $type !== 'impressum' && isset( COMPLIANZ_TC::$config->fields[ $fieldname ]['document_label'] )
			...
			if ( $sync === 'unlink' ) {
				...
				if ( $shortcode ) {
					...
					update_post_meta( $post->ID, 'cmplz_shortcode',
						$post->post_content );
					$document_html
						  = COMPLIANZ_TC::$document->get_document_html( $type,
						$region );
					$args = array(
		...
		public function get_shortcode( $type, $region, $force_classic = false
			...
			if ( ! $force_classic && cmplz_uses_gutenberg()
				...
				$page = COMPLIANZ_TC::$config->pages[ $region ][ $type ];
